...
@Injectable()
export class MomentDateAdapter extends DateAdapter<_moment.Moment> {
  ...
  setLocale(locale: string) {
    ...
    let momentLocaleData = _moment.localeData(locale);
    this._localeData = {
      firstDayOfWeek: momentLocaleData.firstDayOfWeek(),
      longMonths: momentLocaleData.months(),
      shortMonths: momentLocaleData.monthsShort(),
      dates: range(31, (i) => this.createDate(2018, 0, i + 1).format('D')),
  ...
  clone(date: _moment.Moment): _moment.Moment {
      
    return date.clone().locale(this.locale);
  }

  createDateMomentFromString(date:string):_moment.Moment {
  ...
  createDate(year: number, month: number, date: number): _moment.Moment {
    ...
    let result = _moment({year, month, date}).locale(this.locale);
  ...
  today(): _moment.Moment {
    ...
    return _moment().locale(this.locale);
  }

  parse(value: any, parseFormat: string | string[]): _moment.Moment | null {
      
    if (value && typeof value == 'string') {
      return _moment(value, parseFormat, this.locale);
    }
    return value ? _moment(value).locale(this.locale) : null;
  }
  ...
  addCalendarYears(date: _moment.Moment, years: number): _moment.Moment {
    return this.clone(date).add({years});
  }

  addCalendarMonths(date: _moment.Moment, months: number): _moment.Moment {
    return this.clone(date).add({months});
  }

  addCalendarDays(date: _moment.Moment, days: number): _moment.Moment {
  ...
  deserialize(value: any): _moment.Moment | null {
    ...
    if (typeof value === 'string') {
      ...
      date = _moment(value, _moment.ISO_8601).locale(this.locale);
    }
  ...
  invalid(): _moment.Moment {
      
    return _moment.invalid();
  }
}